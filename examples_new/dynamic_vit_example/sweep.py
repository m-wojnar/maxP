#!/usr/bin/env python3
"""LR sweep: SP vs muP vs maxP dynamic (SGD) on CIFAR-10 ViT.
...
Full run:
    python sweep.py --embed-dim 128 --n-layers 4 --steps 2000 --seed 42
"""
...
sys.path.insert(0, str(__import__("pathlib").Path(__file__).resolve().parent.parent / "vit_example"))
from parametrized_vit import ParametrizedViT
...
def train_sp(
    embed_dim, X, Y, *, lr, n_steps, n_layers, mlp_ratio, batch_size, seed,
    alignment="full", desc="",
) -> RunResult:
    """SP baseline: ParametrizedViT with SP (a,b) + SGD + static alignment."""
    torch.manual_seed(seed)
    model = ParametrizedViT(embed_dim=embed_dim, n_layers=n_layers, mlp_ratio=mlp_ratio).to(X.device)
...
def train_mup(
    ...
    """muP baseline: ParametrizedViT with default muP (a,b) + SGD + static alignment."""
    torch.manual_seed(seed)
    model = ParametrizedViT(embed_dim=embed_dim, n_layers=n_layers, mlp_ratio=mlp_ratio).to(X.device)
    param = Parametrization(
        model,
...
def train_maxp(
    ...
    """maxP dynamic: ParametrizedViT + SGD + dynamic alignment solving."""
    torch.manual_seed(seed)
    model = ParametrizedViT(embed_dim=embed_dim, n_layers=n_layers, mlp_ratio=mlp_ratio).to(X.device)
    param = Parametrization(
        model,
    ...
    *,
    ...
    for lr_val in lrs:
        ...
        results["SP"].append(train_sp(
            embed_dim, X, Y, lr=lr_val, n_steps=n_steps,
            n_layers=n_layers, mlp_ratio=mlp_ratio,
...
def plot_sweep(
    ...
    fig.suptitle("LR Sweep: SP vs muP vs maxP dynamic (SGD) — CIFAR-10 ViT", fontsize=14)
...
DEFAULT_LRS = [0.01, 0.03, 0.1, 0.3, 1.0]


def main():
    parser = argparse.ArgumentParser(
        description="LR sweep: SP vs muP vs maxP dynamic (SGD) on CIFAR-10 ViT"
    )
    ...
    print(f"Embed dim: {args.embed_dim}, Layers: {args.n_layers}, Steps: {args.steps}")
    ...
    # ── Summary ──
    print(f"\n{'='*60}")
    print(f"  Best of sweep (dim={args.embed_dim}, layers={args.n_layers}, steps={args.steps}):")
    for method, run in best.items():
