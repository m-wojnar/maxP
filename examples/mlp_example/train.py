...
Usage:
    python examples/mlp_example/train.py
    python examples/mlp_example/train.py --widths 128 256 512
    python examples/mlp_example/train.py --steps 2000 --n-lrs 15
    python examples/mlp_example/train.py --no-plot
"""
...
def main():
    ...
    for width in args.widths:
        ...
        for lr in sp_lrs:
            run += 1
            desc = f"[{run}/{total_runs}] SP  w={width} lr={lr:.1e}"
            sp_loss = train_sp(
                width, X, Y,
                lr=lr,
                n_steps=args.steps,
        ...
        for lr in mup_lrs:
            run += 1
            desc = f"[{run}/{total_runs}] muP w={width} lr={lr:.1e}"
            mup_loss = train_mup(
                width, X, Y,
                lr_prefactor=lr,
                n_steps=args.steps,
