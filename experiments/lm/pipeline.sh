...
RUN_DATE="${RUN_DATE:-}"
# Optional overrides:
#   VAL_DATASET=...      -> passthrough to train.py
#   SEEDS="1 2"          -> narrow seed set (spare GPU)
#   SWEEP_LRS="1e-02 3e-02 1e-01" -> narrow s1/s2 sweep grid
#   STOP_AT_S3=1         -> skip stage E (no s4/s5)
VAL_DATASET="${VAL_DATASET:-}"
SEEDS="${SEEDS:-}"
SWEEP_LRS="${SWEEP_LRS:-}"
SEQ_LEN="${SEQ_LEN:-}"
BATCH_SIZE="${BATCH_SIZE:-}"
STOP_AT_S3="${STOP_AT_S3:-0}"
...
# Optional passthroughs (only appended when set → default behaviour unchanged).
[ -n "$VAL_DATASET" ] && LAUNCH+=(--val-dataset "$VAL_DATASET")
[ -n "$SEQ_LEN" ]     && LAUNCH+=(--seq-len "$SEQ_LEN")
[ -n "$BATCH_SIZE" ]  && LAUNCH+=(--batch-size "$BATCH_SIZE")
[ -n "$SEEDS" ]       && LAUNCH+=(--seeds $SEEDS)
# Narrow the small-scale (s1/s2) sweep grid when requested.
SWEEP_LR_ARGS=()
[ -n "$SWEEP_LRS" ] && SWEEP_LR_ARGS=(--lrs $SWEEP_LRS)
...
"${LAUNCH[@]}" --scale s1 "${SWEEP_LR_ARGS[@]}" --measure-only --tag meas --job-ids-file "$STATE/A.ids"
"${LAUNCH[@]}" --scale s2 "${SWEEP_LR_ARGS[@]}" --measure-only --tag meas --job-ids-file "$STATE/A.ids"
...
for SC in s1 s2; do
    "${LAUNCH[@]}" --scale "$SC" "${SWEEP_LR_ARGS[@]}" --methods maxP-meas \
...
if [ "$STOP_AT_S3" = "1" ]; then
    log "STOP_AT_S3=1 — skipping stage E (s4/s5). Verdict at coord D (E1 @ s3)."
    log "DAG submitted through s3. Verdicts: cat $STATE/report.txt"
    exit 0
fi
