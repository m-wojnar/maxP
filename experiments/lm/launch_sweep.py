...
SCALE_CONFIGS = {
    ...
    "s3": {"wall": "24:00:00", "nodes": 1, "gpus": 1, "ckpt": 5000, "chain": 1},
...
def main() -> None:
    ...
    p.add_argument("--wall", default=None,
                   help="Override SLURM wall time (HH:MM:SS). Needed for small "
                        "batch sizes, whose step count (∝ 1/batch at fixed token "
                        "budget) exceeds the per-scale default. Default: per-scale.")
    ...
    p.add_argument("--val-dataset", default=None,
                   help="Validation dataset passed to train.py. Default: train.py's own "
                        "default (c4_validation).")
    p.add_argument("--steps", type=int, default=None,
                   help="Override training steps passed to train.py.")
    p.add_argument("--seq-len", type=int, default=None,
                   help="Override context length (default: train.py's 3072).")
    ...
    p.add_argument("--weight-decay", type=float, default=None,
                   help="AdamW weight decay passed to train.py. When set, tagged '_wd{v}' "
                        "in the run name. Default: train.py's 0.1.")
    p.add_argument("--indep-wd", action="store_true",
                   help="Independent weight decay (see train.py --indep-wd). "
                        "Run name gains '_iwd'.")
    ...
    wall_time = args.wall or sc["wall"]
        ...
        for lr in lrs:
            for seed in seeds:
                ...
                if args.weight_decay is not None:
                    run_name += f"_wd{args.weight_decay:g}"
                if args.indep_wd:
                    run_name += "_iwd"
                ...
                if args.weight_decay is not None:
                    extra.append(f"--weight-decay {args.weight_decay}")
                if args.indep_wd:
                    extra.append("--indep-wd")
                if args.val_dataset:
                    extra.append(f"--val-dataset {args.val_dataset}")
                if args.steps is not None:
                    extra.append(f"--steps {args.steps}")
                if args.seq_len is not None:
                    extra.append(f"--seq-len {args.seq_len}")
                if sc["ckpt"]:
