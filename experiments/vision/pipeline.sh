...
# bs 256 -> 256*196 = 50176 tokens/step, matching the LM run's 16*3072 = 49152
# (+2.1%; 196 = 4*49 has no exact integer batch for 49152). 1 patch = 1 token.
BATCH="${BATCH:-256}"
...
DRY="${DRY:-0}"
RUN_DATE="${RUN_DATE:-}"
# Per-scale budget. Two modes:
#   * MAXSTEPS_Sx set  -> Chinchilla D=20N tokens (1 patch = 1 token). max_steps =
#     round(20*N / (196*BATCH)); overrides epochs + scales the WSD schedule.
#     Values below assume BATCH=256 (recompute if it changes).
#   * MAXSTEPS_Sx empty -> fall back to EPOCHS_Sx (fixed-budget run).
MAXSTEPS_S1="${MAXSTEPS_S1:-5211}"
MAXSTEPS_S2="${MAXSTEPS_S2:-18570}"
MAXSTEPS_S3="${MAXSTEPS_S3:-73503}"
MAXSTEPS_S4="${MAXSTEPS_S4:-292455}"
MAXSTEPS_S5="${MAXSTEPS_S5:-1166709}"
EPOCHS_S1="${EPOCHS_S1:-1}"
EPOCHS_S2="${EPOCHS_S2:-4}"
EPOCHS_S3="${EPOCHS_S3:-16}"
# Per-scale budget flag: --max-steps N when MAXSTEPS_Sx set, else empty (epochs).
budget() {
    local n v
    n="MAXSTEPS_$(printf '%s' "$1" | tr '[:lower:]' '[:upper:]')"
    v="${!n}"
    [ -n "$v" ] && echo "--max-steps $v"
}
# Tight-budget seeds: 2 at the s1/s2 tuning scales, 1 at the s3 verify scale.
SEEDS_SMALL="${SEEDS_SMALL:-1 2}"
SEEDS_S3="${SEEDS_S3:-1}"
# Skip the s4/s5 stage by default (tight budget).
RUN_STAGE_E="${RUN_STAGE_E:-0}"

# Candidate LR grid — tightened to a 3-pt bracket around the known 3e-2 optimum.
GRID_LRS="${GRID_LRS:-1e-02 3e-02 1e-01}"
...
LAUNCH=(python experiments/vision/launch_sweep.py
        ...
        --dataset "$DATASET" --batch-size "$BATCH")
...
# Scope every analysis glob to this run-date so stale dirs from other
# experiments (e.g. the LM ladder's *_meas runs) are never swallowed.
RG="$RUNS_DIR/${RUN_DATE:-*}_"
...
dep_from() {  # ids file -> afterany:id:id:...  (empty => no dep: parents already done)
    [ "$DRY" = "1" ] && { echo "afterany:DRY"; return; }
    [ -s "$1" ] && echo "afterany:$(paste -sd: "$1")"
}
...
"${LAUNCH[@]}" --scale s1 --methods mup-no --measure-only --tag meas \
    --epochs "$EPOCHS_S1" $(budget s1) --lrs $GRID_LRS --seeds $SEEDS_SMALL --job-ids-file "$STATE/A.ids"
"${LAUNCH[@]}" --scale s2 --methods mup-no --measure-only --tag meas \
    --epochs "$EPOCHS_S2" $(budget s2) --lrs $GRID_LRS --seeds $SEEDS_SMALL --job-ids-file "$STATE/A.ids"

# ============ COORD B: pick source LRs, export tables, gates ================
{
    coord_header vmaxp_coordB "$(dep_from "$STATE/A.ids")"
    cat <<EOF
BEST_S2=\$($ANALYZE best-lr --glob "${RG}s2_mupno_lr*_s*_meas")
BEST_S1=\$($ANALYZE best-lr --glob "${RG}s1_mupno_lr*_s*_meas")
echo "\$BEST_S2" > "$STATE/best_lr_mupno_s2.txt"
echo "\$BEST_S1" > "$STATE/best_lr_mupno_s1.txt"
$EXPORT ${RG}s2_mupno_lr\${BEST_S2}_s*_meas -o "$STATE/s2_align.json"
$EXPORT ${RG}s1_mupno_lr\${BEST_S1}_s*_meas -o "$STATE/s1_align.json"
ADJ_S2=\$(python -c "g='$GRID_LRS'.split(); i=g.index('\$BEST_S2'); print(g[i+1] if i+1 < len(g) else g[i-1])")
{
    ...
    $ANALYZE gates \\
        --runs-glob "${RG}s2_mupno_lr\${BEST_S2}_s*_meas" \\
        --adjacent-glob "${RG}s2_mupno_lr\${ADJ_S2}_s*_meas"
...
# ============ COORD C: pick transfer LR at small scale, release s3 ==========
{
    ...
    cat <<EOF
T_S2=\$($ANALYZE best-lr --glob "${RG}s2_maxPmeas_lr*_s*_transfer-s2")
T_S1=\$($ANALYZE best-lr --glob "${RG}s1_maxPmeas_lr*_s*_transfer-s2")
...
"${LAUNCH[@]}" --scale s3 --methods mup-no --lrs $GRID_LRS --tag base \
    --epochs "$EPOCHS_S3" $(budget s3) --seeds $SEEDS_S3 \
...
# ============ COORD D: transfer proof + E1 verdict, release s4/s5 ===========
{
    ...
    cat <<EOF
BEST3_BASE=\$($ANALYZE best-lr --glob "${RG}s3_mupno_lr*_s*_base")
BEST3_TRAN=\$($ANALYZE best-lr --glob "${RG}s3_maxPmeas_lr*_s*_transfer-s2")
...
if ! $ANALYZE e1 \\
        --baseline-glob "${RG}s3_mupno_lr*_s*_base" \\
        --transfer-glob "${RG}s3_maxPmeas_lr*_s*_transfer-s2" \\
...
# Skipped by default (RUN_STAGE_E=0): s1-s3 only.
if [ "$RUN_STAGE_E" = "1" ]; then
log "stage E: s4 + s5 candidates (per-LR gated by coord D, chained resume)"
"${LAUNCH[@]}" --scale s4 --methods mup-no --lrs $GRID_LRS --tag base \
    $(budget s4) --dependency "afterany:$DJOB" --require "$STATE/s4_go_base_{lr}"
"${LAUNCH[@]}" --scale s4 --methods maxP-meas --alignment-table "$STATE/s2_align.json" \
    --lrs $GRID_LRS --tag transfer-s2 \
    $(budget s4) --dependency "afterany:$DJOB" --require "$STATE/s4_go_transfer_{lr}"
"${LAUNCH[@]}" --scale s5 --methods mup-no --lrs $GRID_LRS --tag base \
    $(budget s5) --dependency "afterany:$DJOB" --require "$STATE/s5_go_base_{lr}"
"${LAUNCH[@]}" --scale s5 --methods maxP-meas --alignment-table "$STATE/s2_align.json" \
    --lrs $GRID_LRS --tag transfer-s2 \
    $(budget s5) --dependency "afterany:$DJOB" --require "$STATE/s5_go_transfer_{lr}"
else
log "stage E (s4/s5) SKIPPED — set RUN_STAGE_E=1 to enable"
fi
